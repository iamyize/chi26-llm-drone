...
# Helper function for audio normalisation
def rms(frame):
    count = len(frame)/2
    # short is 16 bit int
...
# Transcribes the instructions from audio files to text with Google API
def transcribe_audio(frames):
    ...
    for alternative in results["results"][0]["alternatives"]:
        if alternative:
...
# Listens to the user's verbal command and records the audio to for transcription
def listen():
    frames = []
