...
# Sanitises and verifies code generated by ChatGPT to minimise errors
def verify_code(code):
    if "describe_color" in code:
        code = '\n'.join(line for line in code.splitlines() if ("describe_color" in line.lower()))
    elif "count" in code:
    ...
    elif "where_is_exit" in code:
        code = '\n'.join(line for line in code.splitlines() if any(word in line.lower() for word in ["take_off", "land", "where_is_exit"]))
    elif "find_item" in code:
        code = '\n'.join(line for line in code.splitlines() if any(word in line.lower() for word in ["take_off", "land", "find_item"]))

    return code

# Converts natural language instructions to python code for the drone with ChatGPT
